...
_PROJECT_ROOT = _Path("C:\\Users\\rswap\\OneDrive\\Desktop\\project\\testify-automator-ai\\backend\\organizations\\manoj\\137-bank") if "C:\\Users\\rswap\\OneDrive\\Desktop\\project\\testify-automator-ai\\backend\\organizations\\manoj\\137-bank" else None
...
def _poll_task_via_api(title: str, timeout_sec: int = 15, interval_sec: float = 1.0):
    ...
    while _time.time() < deadline:
        ...
        if api_resp.get('status') == 200:
            api_items = api_resp.get('data') or []
            if isinstance(api_items, dict):
                api_items = api_items.get('items') or api_items.get('data') or []
            if isinstance(api_items, list):
                if any((item or {}).get('title') == title for item in api_items if isinstance(item, dict)):
                    return True
            last_payload = api_items
        _time.sleep(interval_sec)
    raise AssertionError(
        f"Task '{title}' not found via API after UI creation. "
        f"Last API payload: {last_payload}"
    )

def _verify_task_in_ui(page, title: str, timeout_ms: int = 6000):
    try:
        locator = page.get_by_text(title, exact=True).first
        expect(locator).to_be_visible(timeout=timeout_ms)
        return True
    except Exception:
        locator = page.get_by_text(re.compile(re.escape(title), re.I)).first
        expect(locator).to_be_visible(timeout=timeout_ms)
        return True

def _try_api_verification(title: str):
    try:
        _poll_task_via_api(title)
    except RuntimeError as exc:
        print(f"[ui_runner] API verification skipped: {exc}")
    return True

def _ensure_api_base_from_network(page):
    # If API base is already set, do nothing.
    for key in ("SMARTAI_API_BASE_URL", "SMARTAI_API_URL", "API_BASE_URL", "API_URL"):
    ...
    def _capture(url: str):
        ...
        if any(token in path for token in ("/todos", "/tasks")):
    ...
    if captured["base"]:
        os.environ["SMARTAI_API_BASE_URL"] = captured["base"]
        print(f"[ui_runner] Detected API base URL: {captured['base']}")

RUN_TAGS = {
    "TS_001_TC_001_add_new_customer_to_crm_positive": [
        "regression"
    ],
    "TS_001_TC_002_add_new_customer_to_crm_negative": [
        "functional"
    ],
    "TS_001_TC_003_add_new_customer_to_crm_edge": [
        "functional"
    ]
}

@pytest.mark.regression
def TS_001_TC_001_add_new_customer_to_crm_positive():
    import time
    import os
    from pathlib import Path as _Path
...
def TS_001_TC_002_add_new_customer_to_crm_negative():
    import time
    import os
    from pathlib import Path as _Path
    with sync_playwright() as p:
...
@pytest.mark.functional
def TS_001_TC_003_add_new_customer_to_crm_edge():
    import time
    import os
    from pathlib import Path as _Path
    with sync_playwright() as p:
...
if __name__ == '__main__':
    ...
    failures = 0
    if _should_run('TS_001_TC_001_add_new_customer_to_crm_positive'):
        try:
            print(f'\n[ui_runner] Running test: TS_001_TC_001_add_new_customer_to_crm_positive...\n')
            run_allure_case('TS_001_TC_001_add_new_customer_to_crm_positive', TS_001_TC_001_add_new_customer_to_crm_positive)
            print(f'\n[ui_runner] TS_001_TC_001_add_new_customer_to_crm_positive: PASS\n')
        except Exception as exc:
            failures += 1
            print(f'\n[ui_runner] TS_001_TC_001_add_new_customer_to_crm_positive: FAIL\nDetails: {exc}\n')
    else:
        print(f'\n[ui_runner] Skipping TS_001_TC_001_add_new_customer_to_crm_positive (tag filter)\n')
    if _should_run('TS_001_TC_002_add_new_customer_to_crm_negative'):
        try:
            print(f'\n[ui_runner] Running test: TS_001_TC_002_add_new_customer_to_crm_negative...\n')
            run_allure_case('TS_001_TC_002_add_new_customer_to_crm_negative', TS_001_TC_002_add_new_customer_to_crm_negative)
            print(f'\n[ui_runner] TS_001_TC_002_add_new_customer_to_crm_negative: PASS\n')
        except Exception as exc:
            failures += 1
            print(f'\n[ui_runner] TS_001_TC_002_add_new_customer_to_crm_negative: FAIL\nDetails: {exc}\n')
    else:
        print(f'\n[ui_runner] Skipping TS_001_TC_002_add_new_customer_to_crm_negative (tag filter)\n')
    if _should_run('TS_001_TC_003_add_new_customer_to_crm_edge'):
        try:
            print(f'\n[ui_runner] Running test: TS_001_TC_003_add_new_customer_to_crm_edge...\n')
            run_allure_case('TS_001_TC_003_add_new_customer_to_crm_edge', TS_001_TC_003_add_new_customer_to_crm_edge)
            print(f'\n[ui_runner] TS_001_TC_003_add_new_customer_to_crm_edge: PASS\n')
        except Exception as exc:
            failures += 1
            print(f'\n[ui_runner] TS_001_TC_003_add_new_customer_to_crm_edge: FAIL\nDetails: {exc}\n')
    else:
        print(f'\n[ui_runner] Skipping TS_001_TC_003_add_new_customer_to_crm_edge (tag filter)\n')
